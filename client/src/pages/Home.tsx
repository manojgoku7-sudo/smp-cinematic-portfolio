    ...
    <main className={`page-shell ${motionPaused ? "motion-paused" : ""} ${lowDataMode ? "low-data" : ""} ${lightPreset ? "contrast-light" : ""} ${recruiterReviewOpen ? "recruiter-review-active" : ""}`}>
      ...
      <section id="work" className="editorial-band top-rule bg-[#0d0b15] py-28 md:py-40" onPointerDown={createProjectPulse} onPointerMove={followProjectFinder} onPointerLeave={() => setProjectFinder((current) => ({ ...current, active: false }))}>
        ...
        <div className="container relative z-10"><Reveal><SectionIntro index="03" eyebrow="Selected work" title="Proof of practice." detail="Two focused case studies across applied machine learning and mobile product design — distinct problems, one bias toward clear decisions." motionPaused={motionPaused} /></Reveal>
          ...
          <Reveal delay={0.18} className="mt-5"><article id="ai-content-studio" className="project-card ai-studio-project panel"><img className="project-art" src="/manus-storage/ai-content-studio-showcase_e194be53.jpg" alt="Abstract autonomous AI media studio with vertical video frames and orbital data streams" /><div className="project-scrim" /><span className="project-signal">autonomous studio</span><span className="project-index">03 / 03</span><div className="relative z-10 flex min-h-[520px] flex-col justify-end p-7 md:p-10"><p className="project-meta label text-violet-200">AI & backend automation · media processing pipelines</p><h3 className="display mt-3 max-w-[18ch] text-4xl leading-[.95] text-white md:text-6xl">YouTube Auto-Uploader & Autonomous AI Content Studio</h3><p className="mt-5 max-w-3xl text-sm leading-6 text-[#d1cadb]">An autonomous Python pipeline for multi-channel monitoring, vertical Shorts creation, AI-assisted metadata and thumbnails, bilingual workflows, publishing schedules, and resilient cloud operation.</p><div className="ai-studio-card-signals"><span><b>Production-ready</b><em>Deployed workflow</em></span><span><b>Lead backend & automation architect</b><em>End-to-end pipeline</em></span><span><b>Python · Gemini · FFmpeg</b><em>AI & media systems</em></span></div><div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-t border-white/15 pt-5"><span className="label text-[0.57rem] text-white/65">Showcase only · No repository or live demo listed</span><button type="button" className="ai-studio-detail-trigger" onClick={() => setAiContentStudioOpen(true)} aria-haspopup="dialog">Open project brief <ArrowUpRight size={15} /></button></div></div></article></Reveal>
        </div>
      </section>
