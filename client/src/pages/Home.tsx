...
function AutomationStudioWalkthrough({ motionPaused, lowDataMode }: { motionPaused: boolean; lowDataMode: boolean }) {
  ...
  const [runId, setRunId] = useState(0);
  const staticPlayback = reduceMotion || motionPaused || lowDataMode;
  return <section className={`studio-walkthrough ${staticPlayback ? "is-static" : ""}`} aria-label="AI Content Studio automation walkthrough"><div className="studio-walkthrough-head"><div><span className="label">Automation walkthrough</span><p>Monitor → produce → scheduled publish</p></div><button type="button" className="studio-replay" onClick={() => setRunId((current) => current + 1)} disabled={staticPlayback} aria-label={staticPlayback ? "Walkthrough motion is currently paused" : "Replay AI Content Studio walkthrough"}><Play size={12} />Replay</button></div><div key={runId} className="studio-console" aria-hidden="true"><div className="studio-stage studio-monitor"><span>01 / SIGNAL MONITOR</span><b>Channel watch</b><div><i /><i /><i /></div><em>New source detected</em></div><div className="studio-stage studio-produce"><span>02 / ASSET ENGINE</span><b>Shorts & metadata</b><div><i /><i /><i /></div><em>Render and enrich</em></div><div className="studio-stage studio-publish"><span>03 / DELIVERY QUEUE</span><b>Scheduled publish</b><div><i /><i /><i /></div><em>Ready for release</em></div><span className="studio-scanline" /></div><p className="studio-walkthrough-note">Illustrative automation preview: source monitoring, asset production, and scheduled publishing in one operational loop.</p></section>;
}
...
export default function Home() {
  ...
  const [gravityProject, setGravityProject] = useState<CaseStudyId | null>(null);
        ...
        <div className="container flex h-[5rem] items-center justify-between">
          <div className="flex items-center gap-5">
            <button className="monogram-trigger flex items-center gap-3 text-left" onClick={() => { triggerMonogramRipple(); scrollToSection("top"); }} aria-label="Go to the top and reveal the MJ monogram" aria-describedby="mj-brand-tooltip">
              <span className={`seal-wrap ${monogramRipple ? "is-rippling" : ""}`} style={{ opacity: sealScrollOpacity }}><span className="monogram-halo" aria-hidden="true" style={{ transform: `rotate(${sealOrbitScrollOffset}deg)` }}><i className="monogram-halo-sweep" /><i key={`section-glint-${monogramSectionGlint || "idle"}`} className={`monogram-section-glint ${monogramSectionGlint ? "is-active" : ""}`} /><i key={`project-echo-${monogramProjectEcho || "idle"}`} className={`monogram-project-echo ${monogramProjectEcho ? `is-active ${openCaseSignal === "attack-study" ? "is-cyan" : "is-violet"}` : ""}`} /><span className={`monogram-selector-preview ${activeOrbitPreview ? `is-active ${activeOrbitPreview === "attack-study" ? "is-cyan" : "is-violet"}` : ""}`}><i className="monogram-selector-preview-cyan" /><i className="monogram-selector-preview-violet" /></span><b className="monogram-orbit-spark" /></span><span className="monogram-click-ripple" aria-hidden="true" /><img src="/manus-storage/smp-mj-monogram-clear-j_24fbf37a.png" alt="MJ monogram" /></span>
      ...
      <section id="work" className="editorial-band top-rule bg-[#0d0b15] py-28 md:py-40" onPointerDown={createProjectPulse} onPointerMove={followProjectFinder} onPointerLeave={() => setProjectFinder((current) => ({ ...current, active: false }))}>
        ...
        <div className="container relative z-10"><Reveal><SectionIntro index="03" eyebrow="Selected work" title="Proof of practice." detail="Two focused case studies across applied machine learning and mobile product design — distinct problems, one bias toward clear decisions." motionPaused={motionPaused} /></Reveal>
          ...
          <div className="project-grid mt-14 grid gap-5 lg:grid-cols-2">
            ...
            <Reveal delay={0.18}><article id="ai-content-studio" className="project-card ai-studio-project panel"><img className="project-art" src="/manus-storage/ai-content-studio-showcase_e194be53.jpg" alt="Abstract autonomous AI media studio with vertical video frames and orbital data streams" /><div className="project-scrim" /><span className="project-signal">autonomous studio</span><span className="project-index">03 / 03</span><div className="relative z-10 flex min-h-[620px] flex-col p-7 md:p-9 ai-studio-card-copy"><p className="project-meta label text-violet-200">AI & backend automation · production workflow</p><h3 className="display mt-3 max-w-[13ch] text-4xl leading-[.93] text-white md:text-5xl">YouTube Auto-Uploader & Autonomous AI Content Studio</h3><p className="mt-5 max-w-[43ch] text-sm leading-6 text-[#d1cadb]">A Python automation system for multi-channel monitoring, vertical Shorts, AI-assisted metadata, bilingual workflows, and resilient publishing operations.</p><div className="ai-studio-outcome"><span className="ai-studio-outcome-orbit" aria-hidden="true"><i /></span><div><p className="label">Outcome marker</p><b>Autonomous content pipeline</b><em>Ingestion → Shorts → metadata → scheduling</em></div></div><AutomationStudioWalkthrough motionPaused={motionPaused} lowDataMode={lowDataMode} /><button type="button" className="ai-studio-detail-trigger ai-studio-brief-action" onClick={() => setAiContentStudioOpen(true)} aria-haspopup="dialog">Open project brief <ArrowUpRight size={15} /></button></div></article></Reveal>
          </div>
        </div>
      </section>
