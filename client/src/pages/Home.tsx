...
const heroVideoUrl = (import.meta.env.VITE_HERO_VIDEO_URL as string | undefined)?.trim() || "/manus-storage/hero-ambient.mp4";
const heroVideoMp4 = heroVideoUrl.endsWith(".mp4") ? heroVideoUrl : heroVideoUrl.replace(/\.webm$/, ".mp4");
const heroVideoWebm = heroVideoUrl.endsWith(".webm") ? heroVideoUrl : heroVideoUrl.replace(/\.mp4$/, ".webm");
...
const education = {
  ...
  coursework: ["Data Structures & Algorithms", "Database Management Systems", "Object-Oriented Programming", "Web Engineering", "Applied Machine Learning"],
} as const;

const skills = [
  { title: "Languages", code: "01", items: ["Java", "JavaScript", "Python", "SQL"] },
  { title: "Frontend", code: "02", items: ["HTML5", "CSS3", "React"] },
...
function downloadResume() {
  ...
  anchor.href = "/resume/S-Manoj-Prabhu-Resume.pdf";
  anchor.download = "S-Manoj-Prabhu-Resume.pdf";
...
export default function Home() {
  ...
  const [lowDataMode, setLowDataMode] = useState(() => typeof window !== "undefined" && window.localStorage.getItem("smp-low-data-preset") === "true");
  ...
  useEffect(() => {
    window.localStorage.setItem("smp-low-data-preset", String(lowDataMode));
  }, [lowDataMode]);
    ...
    <main className={`page-shell ${motionPaused ? "motion-paused" : ""} ${lowDataMode ? "low-data" : ""} ${lightPreset ? "contrast-light" : ""} ${recruiterReviewOpen ? "recruiter-review-active" : ""}`}>
      ...
      <header className={`nav-shell ${scrolled ? "is-scrolled" : ""}`}>
        <div className="container flex h-[5rem] items-center justify-between">
          ...
          <div className="hidden items-center gap-2 md:flex"><button className="contrast-toggle" type="button" onClick={() => setLightPreset((enabled) => !enabled)} aria-pressed={lightPreset}>{lightPreset ? <Moon size={14} /> : <Sun size={14} />}{lightPreset ? "Dark" : "Light"}</button><button className="contrast-toggle" type="button" onClick={() => setLowDataMode((enabled) => !enabled)} aria-pressed={lowDataMode} aria-label={lowDataMode ? "Disable low-data mode" : "Enable low-data mode"}>{lowDataMode ? <WifiOff size={14} /> : <Wifi size={14} />}{lowDataMode ? "Low data" : "Standard"}</button><button className="recruiter-trigger" type="button" onClick={startRecruiterReview}>Recruiter path</button><a href="mailto:manojprabhu0707@gmail.com" className="signal-button min-h-0 px-4 py-2.5">Open correspondence <ArrowUpRight size={14} /></a></div>
        ...
        {mobileOpen ? (
          <div className="border-t border-violet-200/10 bg-[#0b0a12]/95 px-5 py-6 backdrop-blur-xl md:hidden">
            <nav className="flex flex-col gap-5" aria-label="Mobile navigation">
              ...
              <div className="flex gap-3 pt-1"><button className="contrast-toggle" type="button" onClick={() => setLightPreset((enabled) => !enabled)} aria-pressed={lightPreset}>{lightPreset ? <Moon size={14} /> : <Sun size={14} />}{lightPreset ? "Dark preset" : "Light preset"}</button><button className="contrast-toggle" type="button" onClick={() => setLowDataMode((enabled) => !enabled)} aria-pressed={lowDataMode}>{lowDataMode ? <WifiOff size={14} /> : <Wifi size={14} />}{lowDataMode ? "Low data preset" : "Standard data"}</button><button className="recruiter-trigger" type="button" onClick={startRecruiterReview}>Recruiter path</button></div>
          ...
          <div className="relative z-10 pt-4 lg:pt-0">
            ...
            <motion.div initial={reduceMotion ? false : { opacity: 0, y: 20 }} animate={reduceMotion ? {} : { opacity: 1, y: 0 }} transition={{ duration: 0.64, delay: 0.28, ease: [0.23, 1, 0.32, 1] }} className="mt-8 max-w-lg">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <button className="signal-button" onClick={downloadResume}>Retrieve résumé · PDF <Download size={15} /></button>
              </div>
      ...
      <section id="experience" className="editorial-band container py-28 md:py-40">
        ...
        <div className="experience-layout mt-14 grid gap-12 lg:grid-cols-[1.28fr_.72fr] lg:gap-20">
          ...
          <Reveal delay={0.1} className="experience-aside-stack self-start"><ExperienceConnectionMap activeExperience={activeExperience} motionPaused={motionPaused} /><div className="panel p-7 md:p-8"><div className="flex items-center gap-3"><GraduationCap className="text-violet-300" size={20} /><p className="label">Education</p></div><div className="mt-7"><p className="display text-3xl leading-tight text-white">{education.degree}</p><p className="mt-3 text-sm leading-6 text-[#c2bbce]">{education.school}</p><p className="mt-5 border-l border-violet-400 pl-3 text-sm text-violet-200">{education.period} · CGPA {education.cgpa}</p><p className="mt-2 text-sm text-[#8f88a0]">Expected graduation: {education.expectedGraduation}</p></div><div className="mt-8 border-t border-white/10 pt-6"><p className="label">Relevant coursework</p><div className="mt-4 flex flex-wrap gap-2">{education.coursework.map((course) => <span key={course} className="skill-chip" tabIndex={0} aria-label={course}>{course}</span>)}</div></div><div className="mt-8 space-y-3 border-t border-white/10 pt-6"><div className="flex justify-between text-sm text-[#aaa4b7]"><span>HSC</span><span className="text-white">80%</span></div><div className="flex justify-between text-sm text-[#aaa4b7]"><span>SSLC</span><span className="text-white">79%</span></div></div></div></Reveal>
        </div>
      </section>
