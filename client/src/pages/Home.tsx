...
function CollectionSignalThread({ activeIndex, staticMotion }: { activeIndex: number; staticMotion: boolean }) {
  return <span className={`collection-signal-thread thread-tone-${activeIndex + 1} ${staticMotion ? "is-static" : ""}`} aria-hidden="true"><i /><b /></span>;
}

function ProjectCollectionDialog({ project, loading, onOpenChange, motionPaused }: { project: CollectionProject | null; loading: boolean; onOpenChange: (open: boolean) => void; motionPaused: boolean }) {
