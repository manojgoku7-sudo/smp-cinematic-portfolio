...
About black-hole visual QA (2026-08-28): restored the historical `/manus-storage/smp-anime-black-hole_fe55ef2a.mp4` reel to the right side of the About section, using the existing `/manus-storage/smp-hero-orbit_86f3fd46.jpg` poster. The video is muted, looping, inline, motion-controlled through `heroVideoRef`, and layered behind the About copy with a restrained screen blend, vignette, orbit ring, and signal line. It is hidden below 768px and under reduced-motion settings to preserve readability and scrolling. Production build, TypeScript, full-page desktop, and 390px mobile checks passed.

Scroll-reveal refinement QA: the shared Reveal wrapper now applies a restrained 520 ms opacity/translate/scale entrance when each section reaches 12% visibility, with an 8% bottom viewport margin and existing per-item delays preserved for staggered content. A `section-reveal` class adds compositor hints only when motion is allowed; reduced-motion explicitly forces full opacity and no transform. TypeScript and production build passed. Full-page desktop and 390 px mobile captures showed stable section composition, readable content, and no visible clipping or horizontal overflow.

Hero Memoji eye-expression repair QA: source-asset calibration identified the iris centers at approximately left 41.85%, right 58.15%, and 51.7% vertical. The fixed pupil socket backing, clip-path, and mask were aligned to those coordinates; eyelid overlays were moved to the same vertical center and the right eyelid was aligned to the right iris. Pointer tracking remains direct-ref based with the existing bounded translation, eased transition, blink scheduler, and mobile suppression. TypeScript and production build passed. Desktop neutral-expression and 390 px mobile fallback captures were checked after the repair.

Hero Memoji pupil-vs-glint repair QA: live CDP mouse input confirmed the actual pointer path updates both `[data-memoji-pupil]` nodes independently of the glasses glints. At opposite cursor positions, both pupils received matching inline transforms of approximately -5.21 px and +5.21 px while the glints retained their separate lens-reflection transforms. The transient entry loader was also made pointer-transparent so it cannot intercept portrait tracking. Pupil travel was increased to 6.2 px horizontal / 2.5 px vertical for visible response while remaining inside the fixed apertures. TypeScript and production build passed.

Hero Memoji duplicate-pupil cleanup QA: the socket backing and clip/mask footprint were expanded to fully cover the static iris baked into the base portrait before the animated full-source iris is composited above it. The pupil image remains the only foreground iris layer inside each eye window, while the existing single glasses layer and independent glints remain unchanged. Desktop and 390 px mobile captures showed a clean one-iris-per-eye presentation; TypeScript and production build passed.

Hero Memoji pupil-only tracking QA: removed pointer-driven writes for eyebrow, portrait tilt, face shadow, backdrop, and lens-glint variables. The pointer handler now applies transforms only to the two bounded pupil image refs; the delayed full-portrait settle tilt was also disabled so glasses and brows remain stationary. The opaque socket backing and hard clip prevent duplicate baked-in pupils from ghosting. TypeScript and production build passed; desktop and 390px mobile presentation remained stable.
